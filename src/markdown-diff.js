...
function extractBlocks(doc) {
    ...
    const extractFromNode = (node, path) => {
        ...
        if (LEAF_BLOCK_TYPES.indexOf(typeName) >= 0) {
            blocks.push({
                nodes: [...path, node],
            });
            return;
...
export function nodesEqual(a, b) {
    ...
    for (let i = 0; i < aMarks.length; i++) {
        ...
        if (aMark.type.name !== bMark.type.name)
            return false;
        ...
        if (aMarkKeys.length !== bMarkKeys.length)
            return false;
        for (let j = 0; j < aMarkKeys.length; j++) {
            const key = aMarkKeys[j];
            if (key !== bMarkKeys[j])
                return false;
            if (aMarkAttrs[key] !== bMarkAttrs[key])
                return false;
        }
    }
    return true;
...
function childrenEqual(a, b) {
    ...
    for (let i = 0; i < a.childCount; i++) {
        ...
        if (aChild.childCount > 0 || bChild.childCount > 0) {
            if (!childrenEqual(aChild, bChild))
                return false;
        }
    }
    return true;
}
export function blockEqual(a, b) {
...
export function deepEquals(node, other) {
    return nodesEqual(node, other) && childrenEqual(node, other);
}
export function blockDiff(docA, docB) {
...
function isTableRowChange(change) {
    const node = change.A?.nodes[change.A.nodes.length - 1] ??
        change.B?.nodes[change.B.nodes.length - 1];
    return (node?.type.name === "table_row" || node?.type.name === "table_header_row");
}
function isTableHeaderRow(change) {
    const node = change.A?.nodes[change.A.nodes.length - 1] ??
        change.B?.nodes[change.B.nodes.length - 1];
    return node?.type.name === "table_header_row";
}
function getTableRowColumnCount(change) {
    const node = change.A?.nodes[change.A.nodes.length - 1] ??
        change.B?.nodes[change.B.nodes.length - 1];
    return node?.childCount ?? 0;
}
function getParentPath(change) {
...
function sortByColumnCount(group) {
    ...
    const firstAppearanceOrder = new Map();
    let order = 0;
...
function sortTableRows(changes) {
    const result = [];
    let i = 0;
...
export function getDiffDecorations(ctx) {
    const decorations = ctx.get(diffDecorationState.key).decorations;
    if (!decorations) {
        return [];
    }
    const result = [];
    const found = decorations.find(0, undefined, (spec) => spec.decorationType === "diff");
    const filtered = found.filter((deco) => deco !== null);
...
function commonNodes(vnodes, nodes) {
    let rs = [];
...
function trySwitchTable(current, ancestorStack, ancestors) {
...
function trySwitchListItem(current, ancestorStack, ancestors) {
    let parentVNode = ancestorStack[ancestorStack.length - 1];
    let parent = ancestors[ancestors.length - 1];
    let newItem = parentVNode.children.length > 0 && current.type.name == "paragraph";
    if (newItem) {
        ancestorStack.pop();
        let newParent = {
            node: parent,
...
export function buildVNode(blockChanges) {
    let mergeGroups = [];
    let currentMergeGroup = [];
    let previousChangeType = "unchanged";
    let ancestorStack = [];
    for (const change of blockChanges) {
        ...
        let parent = ancestorStack.length > 0 ? ancestorStack[ancestorStack.length - 1] : null;
        if (current.type.name == "table_row" ||
...
function createDecorationForNode(pos, node, changeType, groupIndex) {
    const className = getDecorationClass(changeType);
    if (node.type.name === "heading") {
        const decoration = Decoration.inline(pos + 1, pos + node.nodeSize - 1, {
            class: className,
...
function cloneNodeWithSchema(node, schema) {
    if (node.isText) {
...
function buildNodeFromVNode(vnode, schema) {
    if (vnode.children.length > 0) {
        ...
        const node = nodeType.createChecked(vnode.node.attrs, Fragment.from(children));
        vnode.newNode = node;
        return node;
    }
    else {
...
export function createDiffEditState(originalDoc, modifiedDoc, schema) {
    ...
    for (let groupIndex = 0; groupIndex < mergeGroups.length; groupIndex++) {
        ...
        for (const vnode of group) {
            if (vnode.newNode) {
                ...
                if (pos !== -1) {
                    const decoration = createDecorationForNode(pos, vnode.newNode, vnode.changeType, groupIndex);
...
export function getDecorationClass(type) {
    switch (type) {
        case "delete":
...
function filterDeleteNodeDecoration(diffDecorations, action) {
    const result = [];
...
function markNodesToDelete(treeRoot, decorations) {
    for (const decoration of decorations) {
        const from = decoration.from;
        const to = decoration.to;
        const processedNodes = new Set();
        treeRoot.node.descendants((node, pos, _parent) => {
...
function findTreeNodeByNodeRef(treeRoot, targetNode) {
...
function markEmptyParentsToDelete(treeRoot) {
...
function collectNodesToDelete(treeRoot) {
    const nodes = [];
    function traverse(node) {
        if (node.toDelete) {
            nodes.push(node);
...
function shouldReplaceWholeDoc(treeRoot) {
    if (treeRoot.node.type.name !== "doc")
        return false;
...
export function merge(ctx, action, index, mergeAll) {
    let rs = true;
    ...
    let deleteNodeDecorations = filterDeleteNodeDecoration(processDiffDecorations, action);
    ...
    const sortedNodes = nodesToDelete.sort((a, b) => b.pos - a.pos);
    let tr = state.tr;
...
export function getDiffState(ctx) {
...
export function jumpTo(ctx, index) {
    const view = ctx.get(editorViewCtx);
    const groups = getDiffDecorations(ctx);
    ...
    view.dispatch(tr);
    const dom = view.domAtPos(resolvedPos.pos).node;
    if (!dom)
        return;
    dom.scrollIntoView({
        behavior: "smooth",
...
export function diff(ctx, newContent, originContent) {
    const view = ctx.get(editorViewCtx);
    if (!view) {
    ...
    const currentDoc = originContent === undefined
        ? editorState.doc
